...
def is_hub(hub_name):
    return hub_name in geocode.keys()
...
def modify_hub(hub):
    print_hub_rows([hub])
    choice = raw_input('Modify which attribute (SHORT, FULL, LOC, SITE)? ')
    choice = choice.lower()
    if choice == 'short':
	while True:
	    new_short = raw_input('Enter new short hub name: ')
	...
	index_map = {'full' : 0, 'site' : 2}
        new_full_name = raw_input('Enter new %s name: ' % string_map[choice])
	...
	new_lat = float(raw_input('\tLatitude: '))
	new_long = float(raw_input('\tLongitude: '))
